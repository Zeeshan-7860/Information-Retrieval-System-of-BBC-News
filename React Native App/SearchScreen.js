...
import Sports from '../components/Sports2';
import RNFS from 'react-native-fs';

const initialData = require('../news.json');

const Searchset = ({navigation}) => {
  ...
  const saveDataToJsonFile = async (updatedData) => {
    try {
      const path = RNFS.DocumentDirectoryPath + '/news.json';
      ...
      <Modal
        ...
        <ScrollView>
          <View style={styles.fullDocumentContainer}>
            <Image
              style={styles.Image}
              source={{uri: selectedDocument.header_image}}
            />
            <Text style={styles.detailtitle}>{selectedDocument.title}</Text>
            <Text style={styles.Date}>{selectedDocument.news_post_date}</Text>
            <Text style={styles.Category}>
              Category: {selectedDocument.category}
            </Text>
            <Text style={styles.detcontent}>{selectedDocument.content}</Text>
  ...
  const renderListItem = ({item}) => (
    <View style={styles.listItemContainer}>
      ...
      {isVisible && (
        ...
        {item.tokenFrequency && !isStopword(item.tokenFrequency) ? (
          <Text style={styles.frequency}>
            {JSON.stringify(item.tokenFrequency)}
          </Text>
        ) : (
          <Text style={styles.frequency}>Frequency: N/A</Text>
        )}
      </Text>
      )}
      <TouchableOpacity
  ...
  >
    <View style={styles.container}>
      ...
      <Text style={styles.top}>International News</Text>
